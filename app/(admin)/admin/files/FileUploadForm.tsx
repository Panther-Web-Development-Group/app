...
import { uploadFileAssets } from "./actions"
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        const result = await uploadFileAssets(formData)
    ...
    <div className="space-y-4 rounded-lg border border-(--pw-border) bg-secondary/20 p-6">
      <div>
        ...
        <p className="mt-1 text-sm text-foreground/70">
          Upload documents or other files. You can select multiple files at once.
        </p>
      </div>

      <FileUpload
        label="Select Files"
        accept=".pdf,.doc,.docx,.txt,.csv,.xlsx,.xls,.ppt,.pptx,.zip,.rar"
